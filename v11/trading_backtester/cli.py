...
def main(argv=None):
    ...
    # event stream
    ap.add_argument("--events-out", default=None, help="If set on single-run mode, write event stream as JSONL to this file")
    ...
    # Single run (+ optional event stream)
    overrides = parse_kv_list(args.param, strat.params) if args.param else {}
    signals = strat.run(df, overrides)

    if args.events_out:
        trades, curve, events = backtest_with_events(df, signals, bt_cfg, emit_events=True)
        outp = Path(args.events_out)
        outp.parent.mkdir(parents=True, exist_ok=True)
        with outp.open("w", encoding="utf-8") as f:
            for ev in events:
                f.write(ev.to_json())
                f.write("\n")
        print(f"Wrote events: {outp}")
    else:
